...
from core.train.stats import BatchStats, TrainStats
...
def process_validation_batch(
    train_state: TrainState, train_stats: TrainStats, batch: Batch
) -> None:
...
def prepare_epoch(
    train_state: TrainState, train_stats: TrainStats, train_loader: DataLoader
) -> None:
...
def complete_epoch(
    ...
    should_save_state = early_stopper.should_save_state or checkpointer.should_save_state(
        train_state
    )
...
class Trainer:
    ...
    def _run_validation_epoch(
        ...
        val_pbar = create_progress_bar(val_loader, f"Epoch {train_state.epoch}/{num_epochs} (Val)")
        with torch.no_grad():
            # Randomly select a batch for monitoring (only on master process)
            monitor_batch_idx = None
            if (
                distributed_context.is_master
                and len(val_pbar) > 0
                and train_state.epoch % self._monitor_every_n_epochs == 0
            ):
    ...
    def train(
        ...
        finalise_training(
            train_state, train_stats, self._early_stopper, self._checkpointer, self._tracker
        )
