...
from core.tokeniser.tokens import Token, RegularToken, SpecialToken, SpecialTokenType
...
class TrainedTokeniser:
    ...
    def _init_merger(self,
                     vocab: dict[Token, int],
                     merges: list[tuple[RegularToken, RegularToken]]) -> Merger:
        regular_vocab = {token: vocab[token] for token in vocab
                         if isinstance(token, RegularToken)}
    ...
    def merge(self, tokens: list[Token]) -> list[Token]:
        ...
        for is_merge_eligible, group in groupby(tokens, self._is_merge_eligible):
            if is_merge_eligible:
                regular_group = [token for token in group
                                 if isinstance(token, RegularToken)]
