...
class Point5(BaseModel):
    ...
    def from_tensor(cls, tensor: torch.Tensor) -> "Point5":
        ...
        assert torch.all(
            (tensor[2:5] == 0.0) | (tensor[2:5] == 1.0)
        )  # all pen states must be 0 or 1
    ...
    def to_tensor(self) -> Tensor:
        ...
        return torch.tensor([self.dx, self.dy, *pen_state], dtype=torch.float32)


class Point5Repr(InkRepr):
    ...
    def from_ink(cls, ink: DigitalInk) -> Self:
        ...
        for point in get_stroke_point_iterator(ink).rel_points:
            ...
            points.append(
                Point5(dx=point.x, dy=point.y, pen_up=pen_up, pen_down=pen_down, end=False)
            )
        if points:
