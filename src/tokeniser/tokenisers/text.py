...
from core.tokeniser import RegularToken, RegularTokenType, Token, DiscreteTokeniser
...
class TextTokeniser(DiscreteTokeniser):
    def _create_text_token(self, char: str) -> RegularToken:
        return RegularToken(type=RegularTokenType.TEXT,
                            values=char)
    ...
    def _hybrid_str_repr(self, tokens: list[Token]) -> list[Token | str]:
        ...
        for is_regular, group in grouped_tokens:
            if is_regular:
                regular_tokens = [token for token in group
                                  if isinstance(token, RegularToken)]
                values = [token.values for token in regular_tokens
