...
from core.repr.id import TokenReprType
...
def oov() -> None:
    for tokeniser_id, tokeniser in get_tokeniser_iterator(RESULT_PATH, TOKENISER_TYPES, DELTAS, VOCAB_SIZES):
        # Check if tokeniser has trained component
        if not tokeniser.has_trained:
            print(f"Skipping tokeniser {tokeniser_id!s} (vocab={tokeniser_id.vocab_size}) "
                  f"because it has no trained tokeniser")
            continue
