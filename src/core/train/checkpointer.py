...
class Checkpointer:
    def __init__(
        self, model_paths: ModelPaths, checkpoint_interval: int = 20, max_checkpoints: int = 10
    ):
    ...
    def _prune_checkpoints(self) -> None:
        checkpoint_paths = self._model_paths.list_checkpoint_paths()
    ...
    def save_model(self, train_state: TrainState) -> None:
        model_path = self._model_paths.model_path
        model_path.parent.mkdir(parents=True, exist_ok=True)
        train_state.save_model(model_path)

    def save_state(self, train_state: TrainState) -> None:
        checkpoint_path = self._model_paths.get_checkpoint_path(train_state.epoch)
    ...
    def load_state(self, epoch: int, train_state: TrainState) -> TrainState:
        checkpoint_path = self._model_paths.get_checkpoint_path(epoch)
    ...
    def load_latest_state(self, train_state: TrainState) -> TrainState | None:
        latest_path = self._model_paths.get_latest_checkpoint_path()
