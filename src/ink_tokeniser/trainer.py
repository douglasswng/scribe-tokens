...
class InkBpeTrainer:
    def __init__(self, tokeniser_id: TokeniserId, vocab_size: int):
        self.tokeniser_id = tokeniser_id
        self.vocab_size = vocab_size
        self.token_unicode_bidict = TokenUnicodeBidict()

    def _get_regular_tokens_iterator(
        self, ink_iterator: Iterator[DigitalInk]
    ) -> Iterator[list[RegularToken]]:
        tokeniser = TokeniserFactory.create(self.tokeniser_id)
    ...
    def _get_str_iterator(self, ink_iterator: Iterator[DigitalInk]) -> Iterator[str]:
        for regular_tokens in self._get_regular_tokens_iterator(ink_iterator):
            yield "".join(
                self.token_unicode_bidict.token_to_unicode(token) for token in regular_tokens
            )

    def _get_vocab_and_merges(
        self, hf_vocab: dict[str, int], hf_merges: list[tuple[str, str]]
        ...
        for merge in hf_merges:
            token_merge = (
                self.token_unicode_bidict.unicode_to_token(merge[0]),
    ...
    def _extend_vocab(
        self, vocab: dict[RegularToken, int]
    ) -> dict[SpecialToken | RegularToken, int]:
        special_tokens = [
            SpecialToken(type=SpecialTokenType.START),
    ...
    def _process_tokenizer(self, tokenizer: Tokenizer) -> None:
        ...
        vocab, merges = self._get_vocab_and_merges(
            tokenizer_data["model"]["vocab"], tokenizer_data["model"]["merges"]
        )
