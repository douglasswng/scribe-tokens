...
class EpochStats(BaseModel):
    ...
    def losses(self) -> dict[str, float]:
        ...
        return {k: sum(batch_stats.losses[k]
                       for batch_stats in self.batch_stats) / num_batches
...
class TrainStats(BaseModel):
    ...
    def curr_epoch_stats_formatted(self) -> str:
        return (f"Time Elapsed: {self.time_ellapsed_formatted}\n"
...
if __name__ == "__main__":
    ...
    batch_stats1 = BatchStats(
        losses={"cross_entropy": 2.5, "regularization": 0.1},
        max_grad=1.2,
        lr=0.001
    )
    
    batch_stats2 = BatchStats(
        losses={"cross_entropy": 2.1, "regularization": 0.08},
        max_grad=0.9,
        lr=0.001
    )
    ...
    val_batch_stats = BatchStats(
        losses={"cross_entropy": 2.8, "regularization": 0.12},
        max_grad=0.8,
        lr=0.001
    )
