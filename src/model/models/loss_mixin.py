...
class LossMixin:
    ...
    def _coord_loss(self, mixtures: Tensor, means: Tensor, stds: Tensor, rhos: Tensor,
    ...
    def _pen_loss(self, pen_states: Tensor, pen: Tensor) -> Tensor:
        pen_loss = F.cross_entropy(input=pen_states.transpose(-1, -2),
                                   target=torch.argmax(pen, dim=-1),
                                   reduction='none')
        return pen_loss
